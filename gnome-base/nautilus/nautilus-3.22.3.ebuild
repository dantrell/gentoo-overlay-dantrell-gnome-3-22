...
IUSE="exif gnome +introspection packagekit +previewer selinux sendto tracker vanilla-icon vanilla-icon-grid vanilla-menu vanilla-menu-compress vanilla-rename vanilla-search vanilla-thumbnailer xmp"
...
src_prepare() {
	...
	#		eapply "${FILESDIR}"/${PN}-3.22.0-use-old-icon-grid-and-text-width-proportions.patch
	#	fi
	#fi
	...
	if ! use vanilla-rename; then
		eapply "${FILESDIR}"/${PN}-3.22.2-support-slow-double-click-to-rename.patch
	fi
